...
function Paddle:init(x, y, width, height,marginleft,marginright,_name)
    self._name = _name
    self.x = x
    self.y = y
    self.width = width
...
function Paddle:update(dt)
    ...
    -- enabling horizontal movement
    if self.dx < 0 then
        self.x = math.max(self.marginleft, self.x + self.dx * dt)
    else
        self.x = math.min(self.marginright, self.x + self.dx * dt)
    end
